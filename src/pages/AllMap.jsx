...
const AllMap = () => {
  ...
  useEffect(() => {
    ...
    socket.on("new-checkin", (data) => {
      ...
      toast.info(
        `🚍 มีคน Check-In ที่ป้าย ${data.busStopId}, จำนวน ${data.people} คน`
      );
    });

    const onCreated = ({ checkInId, busStopId, people }) => {
      localStorage.setItem(
        "activeCheckInId",
        JSON.stringify({ checkInId, busStopId, people })
      );
    };
    socket.on("checkin-created", onCreated);

    return () => {
      socket.off("new-checkin");
      socket.off("checkin-created", onCreated);
    };
  }, []);
  ...
  const handleCheckInPopup = () => {
    ...
    if (checkinData && Date.now() - checkinData.timestamp < 60000) {
  ...
  useEffect(() => {
    const handleExpired = (data) => {
      localStorage.removeItem("checkinInProgress");
      const active = JSON.parse(localStorage.getItem("activeCheckInId"));
      if (active && Number(active.checkInId) === Number(data?.checkInId)) {
  ...
  useEffect(() => {
    const handleCancelled = ({ checkInId, message }) => {
      const active = JSON.parse(localStorage.getItem("activeCheckInId"));
      if (active && Number(active.checkInId) === Number(checkInId)) {
        Swal.fire({
          icon: "warning",
          title: "คนขับยกเลิกงาน",
          text: message || "งานของคุณถูกยกเลิก โปรดลองเรียกใหม่อีกครั้ง",
          confirmButtonText: "ตกลง",
        });
        // ล้างสถานะ active เพื่อให้กด Check-In ใหม่ได้
        localStorage.removeItem("activeCheckInId");
        // (ถ้าต้องการ) ตั้ง checkinInProgress cooldown ให้หมดไปเลย:
        localStorage.removeItem("checkinInProgress");
      }
    };
    socket.on("checkin-cancelled", handleCancelled);
    return () => socket.off("checkin-cancelled", handleCancelled);
  }, []);
  ...
  return (
    <div className="flex flex-col items-center h-screen p-4">
      ...
      >
        ...
        {memoizedBusStops.map((stop) => (
          <Marker
            ...
            onClick={(event) => handleMarkerClick(event, stop)}
          >
        ...
        {selectedStop && (
          ...
          >
            <div>
              ...
              <p>
                📍 พิกัด: {selectedStop.latitude}, {selectedStop.longitude}
              </p>
            </div>
          </Popup>
        )}
      ...
      <button
        className="mt-4 bg-green-500 text-white px-6 py-3 rounded-md text-lg hover:bg-green-600"
